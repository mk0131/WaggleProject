                                ...
                                <c:if test="${dto.no_Type == '공지' }">
                                    <div class="accordion__item js-accordion-item">
                                        <div class="accordion-header js-accordion-header">
                                            ...
                                            <div class="notice-ymd"><fmt:formatDate value="${dto.no_Date}"
                                        ...
                                        <div class="accordion-body js-accordion-body">
                                            <div class="accordion-body__contents">
                                                <br/><br/>
                                                    ${dto.no_Content }
                                                <br>
                                                <br/>공고일자 : <fmt:formatDate value="${dto.no_Date}"
                                ...
                                <c:otherwise>
                                    <c:forEach items="${list }" var="dto">
                                        <tr class="Inquiry-content">
                                            <td>${dto.in_Type }</td>
                                            <td><a id="Inquiry-title"
                                                   href="/inquiry/detail?in_Code=${dto.in_Code }">${dto.in_Title }</a>
                                            </td>
                                            <td>${dto.in_Date }</td>
                                            <td>${dto.in_Stat }</td>
                                        </tr>
                            ...
                            <td class="btn1" colspan="2" align="right">
                                <c:if test="${user_Nm ne 'admin'}">
                                    <div class="btn-admin" id="btn-admin">
                                        <button type="button" value="문의" onclick="location.href='/inquiry/insertform'">
                                            문의 작성
                                        </button>
                                    </div>
                                </c:if>
                    ...
                    <c:if test="${user_Nm ne 'admin' }">
                        <ul class="paging">
                            <c:if test="${paging1.prev}">
                                <li class="paging">
                                    <a class="paging-a"
                                       href='<c:url value="/inquiry/list?user_Code=${user_Code }&page=${paging1.startPage-1}"/>'>이전</a>
                                </li>
                            </c:if>
                            <c:forEach begin="${paging1.startPage}" end="${paging1.endPage}" var="num">
                                <li>
                                    <a id="paging-a" class="${paging1.cri.page == num ? 'nowpage' : null }"
                                       href='<c:url value="/inquiry/list?user_Code=${user_Code }&page=${num}"/>'>${num}</a>
                                </li>
                            </c:forEach>
                            <c:if test="${paging1.next && paging1.endPage > 0}">
                                <li>
                                    <a class="paging-a"
                                       href='<c:url value="/inquiry/list?user_Code=${user_Code }&page=${paging1.endPage+1}"/>'>다음</a>
                                </li>
                            </c:if>
                        </ul>
                    </c:if>
                    ...
                    <c:if test="${user_Nm eq 'admin' }">
                        <ul class="paging">
                            <c:if test="${paging2.prev}">
                                <li class="paging">
                                    <a class="paging-a"
                                       href='<c:url value="/inquiry/list?user_Code=${user_Code }&page=${paging2.startPage-1}"/>'>이전</a>
                                </li>
                            </c:if>
                            ...
                            <c:if test="${paging2.next && paging2.endPage > 0}">
                                <li>
                                    <a class="paging-a"
                                       href='<c:url value="/inquiry/list?user_Code=${user_Code }&page=${paging2.endPage+1}"/>'>다음</a>
                                </li>
                            </c:if>
                        </ul>
                    </c:if>
