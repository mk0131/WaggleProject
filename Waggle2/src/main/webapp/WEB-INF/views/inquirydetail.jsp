    ...
    <div class="notice-container">
        <div class="conbox con3">
            ...
            <table class="Inquiry-detail">
                ...
                <tr>
                    <th>내용</th>
                    <td class="inq-detail-td"><textarea id="incontent" rows="10" cols="60" readonly="readonly"
                                                        style="font-family: 'Noto Sans KR', 'Helvetica Neue', Helvetica, Arial, sans-serif;">${dto.in_Content }</textarea>
                    </td>
                </tr>
                <tr>
                    <th>답변상태</th>
                        ...
                        <c:if test="${user_dto.user_Nm eq user_Nm && dto.in_Stat ne '답변완료' }">
                            <c:if test="${user_Nm ne 'admin' }">
                                <button type="button" value="수정"
                                        onclick="location.href='/inquiry/updateform?in_Code=${dto.in_Code}'">수정
                                </button>
                            </c:if>
                        </c:if>
                        <c:if test="${user_Nm eq 'admin' }">
                            <button type="button" value="답변 작성"
                                    onclick="location.href='/inquiry/admininsertform?in_Code=${dto.in_Code}'">답변 작성
                            </button>
                            <button type="button" value="삭제"
                                    onclick="location.href='/inquiry/delete?in_Code=${dto.in_Code}&user_Code=${user_Code }'">
                                삭제
                            </button>
                        </c:if>
