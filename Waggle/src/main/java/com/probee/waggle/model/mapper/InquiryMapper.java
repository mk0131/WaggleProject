...
public interface InquiryMapper {
	...
	@Select(" SELECT in_Code, originNo, grpOrd, in_RCode, in_UCode, in_Type, in_Title, in_Date, in_Content, in_Stat, IN_ANSWER, in_CHK FROM INQUIRY WHERE IN_UCODE = ${user_Code} OR in_UCode = 1 AND in_RCode = ${user_Code} GROUP BY in_Code, originNo ORDER BY originNo DESC, grpOrd ASC LIMIT #{cri.pageStart}, #{cri.perPageNum} ")
